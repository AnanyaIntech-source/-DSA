...
|---|---|
| Simple Program Base | Prints a welcome message |
| Basic Operations | Performs sum, difference and product of two numbers |
| Sum of First 10 Numbers | Uses `while` loop |
| Print Numbers up to 5 | Uses `while` loop |
| Sum of First N Numbers | Uses `for` loop |
| Greater than 10 Check | Uses `if-else` |
| Compare with 10 | Uses `if-else if-else` |
